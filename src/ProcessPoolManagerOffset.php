 ...
 * @package app\drivers
 */
class ProcessPoolManagerOffset
{
    private $iOffsetStart = 0;      //位移起始值
    private $iOffsetEnd = 0;        //位移结束值
    ...
    public function execute()
    {
        $return = false;
        //设置进程池
        if (empty($this->iOffsetEnd) || empty($this->poolSize) || empty($this->dataSize) || !is_callable($this->task)) {
        ...
        while (1) {
            ...
            if ($i + $this->dataSize >= $this->iOffsetEnd) {
                $offset = $this->iOffsetEnd - $i;
                $ibreak = 1;
            } else {
                $offset = $this->dataSize;
            }
            $params = array_merge($this->callbackParams, ['offsetStart' => $i, 'offsetEnd' => $i + $offset]);
            $i += ($this->dataSize + 1);
     ...
     * @throws \Exception
     */
    public function setOffset($iOffsetStart = 0, $iOffsetEnd = 0)
    {
        if (empty($iOffsetEnd)) {
            throw new \Exception("进程池处理数据不能为空");
        }
        $this->iOffsetStart = $iOffsetStart;
        $this->iOffsetEnd = $iOffsetEnd;
